...
This section describes the procedure used to evaluate the classification performance of a PLS-DA model using a **nested cross-validation framework** combined with **iterative feature elimination based on VIP scores**. Although the code returns fold-wise predictions, the final AUC is computed afterwards from these predictions.
...
This ensures that every fold preserves the original class balance between *Control* and *Cancer*, preventing bias during training and evaluation.



**2. Outer cross-validation loop**

The main evaluation is performed through an outer *k*-fold cross-validation:

* In each outer fold, one subset is held out as the **test set**.
* The remaining samples form the **training set**.
* Only the predictions obtained on these outer test sets are used for the final performance estimation (e.g., AUC).

This provides an unbiased assessment of the model’s generalisation capability.



**3. Recursive feature elimination based on VIP**

Within each outer fold, variable selection is performed on the training set through an iterative procedure:
...
2. Predictions are computed for all possible numbers of components up to `ncomp_max`.
3. Classification accuracy is computed for each number of components.

The best-performing number of components (based on mean inner accuracy) is retained.

**3.2 VIP-based elimination**

A full PLS-DA model is then fitted using all inner-optimal parameters.
VIP scores are computed, and a small fraction of the least informative variables (5% or at least one) is removed.
...
The best feature subset and the corresponding optimal number of components are stored for the outer fold.
...
1. A final PLS-DA model is trained on the full training set of that outer fold.
2. Predictions are generated for the outer test set.
3. The predicted probabilities, predicted classes, and true labels are stored.

These results constitute the fold-wise outputs used to compute the final AUC.


**5. AUC computation**

After all outer folds are processed, the test-set predictions from all folds are concatenated.
The **Area Under the ROC Curve (AUC)** is then computed using:
...
plsda_rfe <- function(X, y, positive_class, negative_class, 
                      ncomp_max = 5, outer_folds = 5, inner_folds = 4) {
  ...
  for (o in seq_along(outer_folds_idx)) {
    ...
    internal_acc_history <- data.frame(Nfeatures = integer(), Accuracy = numeric())
    external_acc_history <- data.frame(Nfeatures = integer(), Accuracy = numeric())
    ...
    repeat {
      ...
      for (f in seq_along(inner_idx)) {
        ...
        preds <- predict(model, newdata = Xi_val, ncomp = 1:min(ncomp_max, length(features)))
        preds_mat <- matrix(preds, nrow = nrow(Xi_val), ncol = min(ncomp_max, length(features)))
      ...
      internal_acc_history <- rbind(internal_acc_history,
                                    data.frame(Nfeatures = length(features),
                                               Accuracy = max(mean_acc)))

      # Model with current features
      df_tr <- data.frame(y = y_train_num, X_train[, features, drop = FALSE])
      model_full <- plsr(y ~ ., data = df_tr, ncomp = opt_ncomp, 
                         method = "oscorespls", scale = TRUE)

      test_pred <- predict(model_full, 
                           newdata = X_test[, features, drop = FALSE], 
                           ncomp = opt_ncomp)
      test_class <- ifelse(test_pred > 0.5, 1, 0)
      acc_ext <- mean(test_class == y_test_num)

      external_acc_history <- rbind(external_acc_history,
                                    data.frame(Nfeatures = length(features),
                                               Accuracy = acc_ext))
      ...
      # number of features to eliminate by iteration
      n_drop <- ceiling(max(1, length(features) * 0.05))  # 5% or 1
      
      vip_order <- sort(vip, decreasing = FALSE)
      drop_feat <- names(vip_order)[1:n_drop]
    ...
    # Train
    df_tr_l <- data.frame(y = y_train_num, X_train[, best_features, drop = FALSE])
    final_model <- plsr(y ~ ., data = df_tr_l, ncomp = best_ncomp, 
                        method = "oscorespls", scale = TRUE)
    
    # Test
    test_pred <- predict(final_model, 
                         newdata = X_test[, best_features, drop = FALSE], 
                         ncomp = best_ncomp)
    ...
    results[[o]] <- list(
      ncomp = best_ncomp,
      n_features = length(best_features),
...
evaluate_models <- function(df_mod, meta, seed=NULL) {
  ...
  nfeat_raw <- round(mean(do.call(rbind, lapply(res_raw, function(x) x$n_features))), 0)
  common_features_raw <- Reduce(intersect, lapply(res_raw, function(x) x$best_features))
  ...
  nfeat_corr <- round(mean(do.call(rbind, lapply(res_corr, function(x) x$n_features))), 0)
  common_features_corr <- Reduce(intersect, lapply(res_corr, function(x) x$best_features))
  ...
  return(list(
    ...
    n_features = list(RAW = nfeat_raw, CORRECTED = nfeat_corr),
    common_features = list(RAW = common_features_raw, CORRECTED = common_features_corr)
  ))
}
```
...
results_auc <- bplapply(clusters, function(cl) {

  purrr::map_dfr(perturb_levels, function(p) {
    ...
    tibble::tibble(
      ...
      Raw_nfeat = as.numeric(res$n_features["RAW"]),
      Corrected_nfeat = as.numeric(res$n_features["CORRECTED"]),
      Raw_features = res$common_features["RAW"],
      Corrected_features = res$common_features["CORRECTED"]
    )
  })
...
We compute the AUC with and without orthogonal correction across 200 repetitions for a single cluster (`Cluster48`) perturbation of 20% and visualize the distribution.

```{r repeated-simulations, warning=FALSE, message=FALSE}
set.seed(NULL)  # No seed fixation
n_repeats <- 200
target_cluster <- "Cluster48"

robust_results <- bplapply(1:n_repeats, function(i) { 
  
  #  randomitza només les etiquetes 
  set.seed(1000 + i)  # nova llavor cada iteració
  df$Group <- sample(rep(c("Control", "Cancer"), length.out = nrow(df)))
  
  #  aplica el desplaçament 
  df_mod <- simulate_shift(df, cluster = target_cluster, perc = 0.20)
  ...
  tibble::tibble(
    Iteration = i,
    Raw = as.numeric(res$auc["RAW"]),
    Corrected = as.numeric(res$auc["CORRECTED"])
  )
}) %>% dplyr::bind_rows()
...
results_global <- bplapply(clusters, function(cl) {
