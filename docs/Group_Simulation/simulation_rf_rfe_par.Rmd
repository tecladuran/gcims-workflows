...
rf_rfe <- function(X, y, positive_class, negative_class,
                   outer_folds = 5, inner_folds = 4,
                   ntree = 500, mtry_grid = c(2, 4, 6), drop_rate = 0.05) {
  ...
  for (o in seq_along(outer_folds_idx)) {
    ...
    internal_acc_history <- data.frame(Nfeatures = integer(), 
                                       Accuracy = numeric())
    external_acc_history <- data.frame(Nfeatures = integer(), 
                                       Accuracy = numeric())
    
    repeat {
      # --- Inner cross-validation for mtry selection ---
      ...
      for (f in seq_along(inner_idx)) {
        ...
        for (m in seq_along(mtry_grid)) {
          rf_model <- randomForest(x = Xi_tr, y = yi_tr,
                                   ntree = ntree, mtry = mtry_grid[m])
          preds <- predict(rf_model, newdata = Xi_val)
          acc_mat[f, m] <- mean(preds == yi_val)
        }
      }
      ...
      # --- Train final model on all training data with optimal mtry ---
      model_full <- randomForest(x = X_train[, features, drop = FALSE],
                                 y = y_train,
                                 ntree = ntree,
                                 mtry = opt_mtry,
                                 importance = TRUE)
      
      preds_test <- predict(model_full, 
      ...
      # --- Variable importance ---
      imp <- importance(model_full, type = 2)[, 1]  # numeric vector
      imp <- sort(imp, decreasing = FALSE)  # ordena de menys a més important
      ...
      # --- Drop percentatge de pitjors variables ---
      n_drop <- ceiling(max(1, length(features) * drop_rate))
      drop_feat <- names(imp)[1:n_drop]  # agafa les menys importants
      features <- setdiff(features, drop_feat)
    }
    
    # --- Final model for this fold ---
    final_model <- randomForest(x = X_train[, best_features, drop = FALSE],
                                y = y_train,
                                ntree = ntree,
                                mtry = best_mtry)
...
evaluate_models <- function(df_mod, meta, seed=NULL) {
  ...
  # --- RAW DATA ---
  res_raw <- rf_rfe(X_raw, y, positive_class = "Cancer", negative_class = "Control")
  
  nfeat_raw <- round(mean(do.call(rbind, lapply(res_raw, function(x) x$n_features))), 0)
  mtry_raw <- round(mean(do.call(rbind, lapply(res_raw, function(x) x$best_mtry))), 0)
  ...
  res_corr <- rf_rfe(X_corr, y, positive_class = "Cancer", negative_class = "Control")
  
  nfeat_corr <- round(mean(do.call(rbind, lapply(res_corr, function(x) x$n_features))), 0)
  mtry_corr <- round(mean(do.call(rbind, lapply(res_corr, function(x) x$best_mtry))), 0)
  ...
  return(list(
    ...
    mtry = list(RAW = mtry_raw, CORRECTED = mtry_corr),
...
results_auc <- bplapply(clusters, function(cl) {
  purrr::map_dfr(perturb_levels, function(p) {
    ...
    tibble::tibble(
      ...
      Raw_mtry = as.numeric(res$mtry$RAW),
      Corrected_mtry = as.numeric(res$mtry$CORRECTED),
      Raw_nfeat = as.numeric(res$n_features$RAW),
      Corrected_nfeat = as.numeric(res$n_features$CORRECTED),
      Raw_features = list(res$common_features$RAW),
      Corrected_features = list(res$common_features$CORRECTED)
    )
  })
})
...
mtry_long <- auc_df %>%
  tidyr::pivot_longer(
    cols = c(Raw_mtry, Corrected_mtry),
    names_to = "Condition",
    values_to = "mtry"
  )
```
...
unique(auc_df$Cluster) %>% purrr::walk(function(cl) {
  ...
  p <- ggplot(df_plot, aes(x = Percent)) +
    ...
    labs(
      title = paste("Classifier AUC vs Perturbation —", cl),
...
We compute the AUC with and without orthogonal correction across 200 repetitions for a single cluster (`Cluster48`) perturbation of 30% and visualize the distribution.
...
robust_results <- bplapply(1:n_repeats, function(i) {
  # nova llavor per a cada iteració (independent per worker)
  set.seed(1000 + i)
  
  # randomitza etiquetes
  df$Group <- sample(rep(c("Control", "Cancer"), length.out = nrow(df)))
...
results_global <- bplapply(clusters, function(cl) {
  purrr::map_dfr(perturb_seq, function(p) {
    purrr::map_dfr(1:n_repeats, function(i) {
      # llavor específica per iteració
      set.seed(1000 + i)
