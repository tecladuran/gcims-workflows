...
knn_rfe_cv <- function(X, y, positive_class, negative_class,
                       outer_folds = 5, inner_folds = 4,
                       k_grid = c(3, 5, 7, 9), drop_rate = 0.05, scale_data = TRUE) {
  ...
  for (o in seq_along(outer_folds_idx)) {
    ...
    repeat {
      # --- Inner CV for k selection ---
      inner_idx <- make_folds(y_train, inner_folds)
      acc_mat <- matrix(NA, nrow = inner_folds, ncol = length(k_grid))
      ...
      for (f in seq_along(inner_idx)) {
        ...
        for (k_idx in seq_along(k_grid)) {
          preds <- knn(train = Xi_tr, test = Xi_val, cl = yi_tr, k = k_grid[k_idx])
          acc_mat[f, k_idx] <- mean(preds == yi_val)
        }
      }
      
      mean_acc <- colMeans(acc_mat, na.rm = TRUE)
      opt_k <- k_grid[which.max(mean_acc)]
      internal_acc_history <- rbind(internal_acc_history,
                                    data.frame(Nfeatures = length(features),
                                               Accuracy = max(mean_acc)))
      
      # --- Final model on full training set ---
      preds_test <- knn(train = X_train[, features, drop = FALSE],
                        test = X_test[, features, drop = FALSE],
                        cl = y_train, k = opt_k, prob = TRUE)
      ...
      # --- Estimate variable importance (proxy via logistic regression) ---
      if (length(features) <= 10) break
      model_glm <- glm(y_train_num ~ ., data = as.data.frame(X_train[, features, drop = FALSE]),
                       family = binomial)
      imp <- abs(coef(model_glm)[-1])
      imp <- sort(imp, decreasing = FALSE)
      
      n_drop <- ceiling(max(1, length(features) * drop_rate))
      drop_feat <- names(imp)[1:n_drop]
      features <- setdiff(features, drop_feat)
    }
    
    # --- Final model for this fold ---
    final_pred <- knn(train = X_train[, best_features, drop = FALSE],
                      test = X_test[, best_features, drop = FALSE],
                      cl = y_train, k = best_k, prob = TRUE)
    probs <- ifelse(final_pred == positive_class,
                    attr(final_pred, "prob"),
                    1 - attr(final_pred, "prob"))
    test_class <- ifelse(final_pred == positive_class, 1, 0)
...
evaluate_models <- function(df_mod, meta, seed=NULL) {
  ...
  # --- RAW DATA ---
  res_raw <- knn_rfe_cv(X_raw, y, positive_class = "Cancer", negative_class = "Control")
  
  nfeat_raw <- round(mean(do.call(rbind, lapply(res_raw, function(x) x$n_features))), 0)
  kneigh_raw <- round(mean(do.call(rbind, lapply(res_raw, function(x) x$best_k))), 0)
  ...
  # --- CORRECTED DATA ---
  X_corr <- orthogonal_correction(data = X_raw,
                                  variables = meta %>% dplyr::select(elapsed_time, batch))$corrected
  
  res_corr <- knn_rfe_cv(X_corr, y, positive_class = "Cancer", negative_class = "Control")
  
  nfeat_corr <- round(mean(do.call(rbind, lapply(res_corr, function(x) x$n_features))), 0)
  kneigh_corr <- round(mean(do.call(rbind, lapply(res_corr, function(x) x$best_k))), 0)
  ...
  return(list(
    ...
    neighbours = list(RAW = kneigh_raw, CORRECTED = kneigh_corr),
...
for (cl in clusters) {
  cluster_auc <- purrr::map_dfr(perturb_levels, function(p) {
    ...
    tibble::tibble(
      ...
      Raw_neigh = as.numeric(res$neighbours["RAW"]),
      Corrected_neigh = as.numeric(res$neighbours["CORRECTED"]),
...
neigh_long <- auc_df %>%
  tidyr::pivot_longer(
    cols = c(Raw_neigh, Corrected_neigh),
    names_to = "Condition",
    values_to = "Neighbours"
  )
...
We compute the AUC with and without orthogonal correction across 200 repetitions for a single cluster (`Cluster14`) perturbation of 20% and visualize the distribution.

```{r repeated-simulations, warning=FALSE, message=FALSE}
n_repeats <- 200
target_cluster <- "Cluster19"
...
robust_results <- purrr::map_dfr(1:n_repeats, function(i) { 
  ...
  df_mod <- simulate_shift(df, cluster = target_cluster, perc = 0.2)
  
  # --- avalua models (folds constants) ---
  res <- evaluate_models(df_mod, meta)
...
results_global <- purrr::map_dfr(clusters, function(cl) {
  purrr::map_dfr(perturb_seq, function(p) {
    purrr::map_dfr(1:n_repeats, function(i) {
      # --- Randomitza les etiquetes ---
      df$Group <- sample(rep(c("Control", "Cancer"), length.out = nrow(df)))
      
      # --- Aplica el desplaçament ---
      df_mod <- simulate_shift(df, cluster = cl, perc = p)
      
      # --- Avalua els models (RAW i CORRECTED) ---
      res <- evaluate_models(df_mod, meta)
