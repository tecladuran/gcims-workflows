...
# devtools::load_all("/storage/users/emartin/GCIMS")
```
...
ggplot(results, aes(x = ampliation, y = MinValue, color = factor(SampleID), group = SampleID)) +
  geom_line() +
  geom_point() +
  labs(title = "Minimum Value vs Ampliation (Cluster 1)",
       x = "Ampliation (%)",
       y = "Minimum Value",
       color = "Sample ID") +
  theme_minimal()+
...
ggplot(results, aes(x = ampliation, y = MinValue, color = factor(SampleID), group = SampleID)) +
  geom_line() +
  geom_point() +
  labs(title = "Minimum Value vs Ampliation (Cluster 14)",
       x = "Ampliation (%)",
       y = "Minimum Value",
       color = "Sample ID") +
  theme_minimal()+
...
ggplot(mean_results, aes(x = ampliation, y = MeanMinValue, color = factor(cluster), group = cluster)) +
  geom_line() +
  geom_point() +
  labs(title = "Mean Minimum Value vs Ampliation per Cluster",
       x = "Ampliation (%)",
       y = "Mean Minimum Value",
       color = "Cluster") +
  theme_minimal()+
  ylim(0, 800)
...
Based on these plots, we will adopt the **200%** ampliation value (i.e., double the size of the ROI), as most clusters appear to have stabilized at a value that we will consider the baseline.
