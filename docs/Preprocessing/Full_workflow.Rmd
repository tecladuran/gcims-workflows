---
title: "Full Workflow: From Raw Data to Peak Table"
...
Enable or disable parallelization based on the need for detailed error reporting or faster execution.
...
annotations <- read.csv("/storage/users/tduran/Projects/gcims-workflows/data/annotations_pool.csv")
...
```{r pre-alignment-2-rics,  echo=FALSE}
print(plot_ric_2(urine))
```
...
```{r post-alignment-rics, echo=FALSE}
print(plot_ric_2(urine))
```
...
**VISUALIZATION OF THE FINAL ROIS**

```{r, echo=FALSE}
# Seleccionem la mostra a visualitzar
sample_plot <- plot_sample(urine$getSample(1), colormap)

# Extreiem límits dels clusters
cluster_limits <- peak_clustering$cluster_stats%>% filter(cluster %in% selected_clusters) %>%
  dplyr::select(cluster, dt_min_ms, dt_max_ms, rt_min_s, rt_max_s)

# Calculem punts centrals per les etiquetes
cluster_limits <- cluster_limits %>%
  dplyr::mutate(
    rt_center = (rt_min_s + rt_max_s) / 2,
    dt_center = (dt_min_ms + dt_max_ms) / 2
  )

cluster_limits_num <- cluster_limits %>%
  mutate(cluster_id = gsub("Cluster", "", cluster))  # treu la paraula i deixa el número


# Fem la figura
roi_plot <- sample_plot +
  ggplot2::geom_rect(
    data = cluster_limits,
    aes(xmin = dt_min_ms, xmax = dt_max_ms, ymin = rt_min_s, ymax = rt_max_s),
    fill = NA, color = "white", linewidth = 0.7
  ) +
  ggplot2::geom_text(
    data = cluster_limits_num,
    aes(y = rt_center+40, x = dt_center, label = cluster_id),
    color = "white", size = 2.5
  )+
  coord_cartesian(xlim = c(6.5, max(dtime(urine))), ylim= c(100,1050))

# Mostrem a l'RStudio
print(roi_plot)

# Guardem amb alta qualitat
ggplot2::ggsave(
  filename = "sample_urine_pool_rois.png",
  plot = roi_plot,
...
p<- ggplot2::geom_rect(
    data = cluster_limits,
    aes(xmin = rt_min_s, xmax = rt_max_s, ymin = dt_min_ms, ymax = dt_max_ms),
...
During peak integration in GC-IMS, the signal is quantified over a fixed Region of Interest (ROI), which may include background signal not related to the actual compound. This can lead to systematic overestimation of peak volumes and misrepresent the true stability of the signals.

To correct for this effect, we apply a cluster-specific baseline correction based on an expanded ROI window that captures the local background. The corrected values are obtained by subtracting this estimated baseline from the imputed peak table.

Details of the method are described in the [baseline correction report](https://github.com/tecladuran/gcims-workflows/blob/main/docs/Preprocessing/baseline_correction.md).
