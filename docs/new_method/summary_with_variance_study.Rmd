...
df_var_batch <- data.frame(
  PC = paste0("PC", 1:5),
  Variance = var_pca_batch[1:5] * 100
)

p_batch <- ggplot(df_var_batch, aes(x = PC, y = Variance)) +
  geom_bar(stat = "identity", fill = "navy") +
  geom_text(aes(label = sprintf("%.1f%%", Variance)),
            vjust = -0.5, size = 2.5) +
  labs(
    title = "Explained variance of PCA on batch-reconstructed data",
...
df_var_acq <- data.frame(
  PC = paste0("PC", 1:5),
  Variance = var_pca_order[1:5] * 100
)

p_acq <- ggplot(df_var_acq, aes(x = PC, y = Variance)) +
  geom_bar(stat = "identity", fill = "navy") +
  geom_text(aes(label = sprintf("%.1f%%", Variance)),
            vjust = -0.5, size = 2.5) +
  labs(
    title = "Explained variance of PCA on acquisition-reconstructed data",
    x = "Principal Components",
    y = "Variance Explained (%)"
  ) +
  theme_minimal(base_size = 10) +
